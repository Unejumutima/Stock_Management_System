...
import { exportMonthlyReportXlsx } from '../utils/exportReport'
...
export default function Reports() {
  ...
  const [customTo, setCustomTo] = useState('2026-05-16')
  const [previewOpen, setPreviewOpen] = useState(false)

  const period: ReportPeriod = useMemo(
    () =>
      useCustomRange
        ? { month: Number(month), year: Number(year), customFrom, customTo }
        : { month: Number(month), year: Number(year) },
    [month, year, useCustomRange, customFrom, customTo],
  )

  const report = useMemo(() => buildReportSnapshot(period), [period])

  const handleExport = () => exportMonthlyReportXlsx(report)
  ...
  return (
    ...
    >
      ...
      <ReportFilters
        ...
        onToggleCustom={setUseCustomRange}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard compact title="Total revenue" value={formatCurrency(report.totalRevenue)} sub={report.periodLabel} icon={<IconRevenue />} />
        <KpiCard compact title="Total expenses" value={formatCurrency(report.totalExpenses)} sub="Purchases + operating" icon={<IconExpense />} />
        <KpiCard compact title="Gross profit" value={formatCurrency(report.totalProfit)} sub="After product COGS" icon={<IconProfit />} />
        <KpiCard compact title="Net profit" value={formatCurrency(report.netProfit)} sub="Revenue − all expenses" icon={<IconNet />} />
      </section>
      ...
      <ExportSection onExport={handleExport} onPreview={() => setPreviewOpen(true)} periodLabel={report.periodLabel} />

      {previewOpen ? <ReportPreviewModal report={report} onClose={() => setPreviewOpen(false)} onExport={handleExport} /> : null}
    </AppLayout>
  )
}
    ...
    <section className={`${panelClass} !p-4 sm:!p-5`}>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <FilterSelect label="Month" value={month} onChange={onMonthChange} disabled={useCustomRange} options={REPORT_MONTHS.map((m) => ({ value: m.value, label: m.label }))} />
          <FilterSelect label="Year" value={year} onChange={onYearChange} disabled={useCustomRange} options={REPORT_YEARS.map((y) => ({ value: y, label: y }))} />
        ...
        {useCustomRange ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:max-w-md">
            <label className="block">
              ...
              <input type="date" value={customFrom} onChange={(e) => onCustomFromChange(e.target.value)} className={fieldClass} />
            </label>
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium text-slate-600">To</span>
              <input type="date" value={customTo} onChange={(e) => onCustomToChange(e.target.value)} className={fieldClass} />
            </label>
          </div>
        ) : null}
      </div>
    ...
    <article className={panelClass}>
      ...
      <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-500">
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2.5 rounded-sm bg-[#0B2735]" /> Revenue
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2.5 rounded-sm bg-slate-300" /> Expenses
        </span>
      </div>
    </article>
  )
...
function ProfitTrendChart({ points }: { points: { label: string; profit: number }[] }) {
  ...
  const coords = points.map((p, i) => {
    ...
    const y = pad + (1 - (p.profit - minP) / range) * (h - pad * 2)
    return { x, y, label: p.label, profit: p.profit }
  })
    ...
    <article className={panelClass}>
      ...
      <div className="relative h-[220px] overflow-hidden rounded-xl bg-gradient-to-b from-slate-50/80 to-white">
        <svg viewBox={`0 0 ${w} ${h}`} className="size-full" preserveAspectRatio="none" aria-hidden>
          ...
          {coords.map((c) => (
            <circle key={c.label} cx={c.x} cy={c.y} r="4" fill="#fff" stroke="#10b981" strokeWidth="2" />
          ))}
        </svg>
      ...
      <p className="mt-3 text-xs text-slate-500">Monthly net profit after purchases and operating expenses.</p>
    </article>
  )
}

function DetailedReportTables({ report }: { report: ReturnType<typeof buildReportSnapshot> }) {
  return (
    <section className="space-y-6">
      <TableSection title="Product performance summary" description="Revenue, COGS, and margin by SKU for the selected period.">
        <DataTable
          ...
          rows={report.productPerformance.map((r) => [
            r.name,
            r.sku,
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <TableSection title="Expense breakdown" description="Operating costs and inventory purchases.">
          <DataTable
            ...
            rows={report.expenseBreakdown.map((r) => [r.category, formatCurrency(r.amount), `${r.sharePct.toFixed(1)}%`])}
            compact
      ...
      <TableSection title="Profit breakdown per product" description="Gross profit contribution by product line.">
        <DataTable
          headers={['Product', 'SKU', 'Revenue', 'Gross profit', 'Margin']}
          rows={report.profitByProduct.map((r) => [
            r.name,
            r.sku,
            formatCurrency(r.revenue),
            formatCurrency(r.grossProfit),
    ...
    <div className="overflow-x-auto">
      <table className={`w-full border-collapse text-left text-sm ${compact ? '' : 'min-w-[640px]'}`}>
        <thead className="border-b border-slate-200/80 bg-slate-50/95">
          <tr>
            {headers.map((h) => (
              <th key={h} className="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 first:pl-5">
                {h}
              </th>
            ))}
          </tr>
          ...
          {rows.map((row, ri) => (
            <tr key={ri} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/80">
              {row.map((cell, ci) => (
                <td
                  key={ci}
                  className={`px-4 py-3.5 text-slate-700 first:pl-5 ${ci === 0 && !compact ? 'font-medium text-slate-800' : ''} ${ci > 0 && ci >= row.length - 2 ? 'tabular-nums' : ''}`}
                >
                  {cell}
                </td>
              ))}
      ...
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-lg">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Download a multi-sheet Excel workbook for <span className="font-medium text-slate-700">{periodLabel}</span> including
            summary, product performance, expenses, and profit analysis.
          </p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <button type="button" className={btnSecondaryClass} onClick={onPreview}>
            Preview report
          </button>
          <button type="button" className={btnPrimaryClass} onClick={onExport}>
            <DownloadIcon className="size-4" />
            Download Excel (.xlsx)
          </button>
        </div>
...
function ReportPreviewModal({
  ...
  report: ReturnType<typeof buildReportSnapshot>
  onClose: () => void
  onExport: () => void
}) {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-4 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="report-preview-title">
      <button type="button" className="absolute inset-0 bg-slate-900/45 backdrop-blur-sm" aria-label="Close" onClick={onClose} />
      <div className="relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-[0_24px_64px_-16px_rgba(15,23,42,0.28)] ring-1 ring-slate-200/80">
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
          <div>
            <h2 id="report-preview-title" className="text-lg font-semibold text-[#0B2735]">
              Report preview
            </h2>
            <p className="mt-0.5 text-sm text-slate-500">{report.periodLabel}</p>
          </div>
        ...
        <div className="overflow-y-auto px-6 py-5">
          <div className="grid gap-3 sm:grid-cols-2">
            <PreviewStat label="Total revenue" value={formatCurrency(report.totalRevenue)} />
            <PreviewStat label="Total expenses" value={formatCurrency(report.totalExpenses)} />
            <PreviewStat label="Gross profit" value={formatCurrency(report.totalProfit)} />
            <PreviewStat label="Net profit" value={formatCurrency(report.netProfit)} highlight />
          </div>
          <div className="mt-6 space-y-4 border-t border-slate-100 pt-5 text-sm">
            <PreviewRow label="Products with sales" value={String(report.productPerformance.length)} />
            <PreviewRow label="Top seller" value={report.topSelling[0]?.name ?? '—'} />
            <PreviewRow label="Largest expense category" value={report.expenseBreakdown[0]?.category ?? '—'} />
            <PreviewRow label="Sheets in export" value="Summary, Performance, Top Selling, Expenses, Profit, Trend" />
          </div>
        </div>
...
function PreviewStat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-xl border px-4 py-3 ${highlight ? 'border-emerald-200 bg-emerald-50/60' : 'border-slate-200/80 bg-slate-50/50'}`}>
      <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{label}</p>
      <p className={`mt-1 text-lg font-semibold tabular-nums ${highlight ? 'text-emerald-800' : 'text-[#0B2735]'}`}>{value}</p>
    </div>
  )
}
